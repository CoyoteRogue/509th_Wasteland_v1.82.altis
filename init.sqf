...
if (!isDedicated) then
{
	[] spawn
	{
		...
		{
			9999 cutText ["Welcome to 509th TACTICAL A3Wasteland, please wait for your client to initialize", "BLACK", 0.01];
...
if (isServer) then
{
	...
	diag_log "509th TACTICAL WASTELAND SERVER - Initializing Server";
...
if (hasInterface || isServer) then
{
	...
	[] execVM "addons\outOfBounds\outOfBounds.sqf";				// Prevents players flying off of the map
	[] execVM "addons\scripts\logo.sqf";                  		// Logo
	[] execVM "addons\playerJump\playerJump.sqf";				// Players jump key will be a combination of SHIFT and 'STEP OVER' (default 'v' key) 	
};
...
[2, 500, 1000] execVM "addons\scripts\MAD_traffic.sqf";